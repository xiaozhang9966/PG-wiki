...
public class EbookController {
    ...
    public CommonResp list(EbookReq req){
        CommonResp<PageResp<EbookResp>> resp = new CommonResp<>();//new Com 返回的参数类型是List集合的Ebook这个实体类的各种数据类型
        PageResp<EbookResp> list = ebookService.list(req);//调用ebookServerice的List方法 返回给list这个变量 类型是List集合的Ebook
